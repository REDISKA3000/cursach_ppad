...
class OpenAIProvider:
    ...
    def _call_chat(
        ...
        try:
            ...
            logger.debug(f"✓ Response from {selected_model}: {len(content)} chars")
            return content
        except Exception as e:
    ...
    def _call_response_text(
        ...
        *,
        ...
        try:
            ...
            logger.debug(f"✓ Responses API output from {model}: {len(content)} chars")
            return content
        except Exception as e:
    ...
    def parse_candidate_profile(self, resume_text: str) -> CandidateProfile:
        ...
        if not self.enabled:
            logger.debug("Using fallback: OpenAI disabled for CandidateProfile")
            fallback_profile = self.fallback.parse_candidate_profile(prepared.normalized_text)
            if prepared.was_truncated:
                fallback_profile.raw_warnings.append("CandidateProfile input was section-truncated before extraction")
            final_profile = normalize_candidate_profile(fallback_profile, prepared.normalized_text, prepared)
            final_profile = enrich_candidate_profile_with_sections(final_profile, prepared)
            logger.info(
        ...
        try:
            ...
            if prepared.was_truncated:
                parsed_profile.raw_warnings.append("CandidateProfile input was section-truncated before extraction")

            final_profile = normalize_candidate_profile(parsed_profile, prepared.normalized_text, prepared)
            final_profile = enrich_candidate_profile_with_sections(final_profile, prepared)
            logger.debug("CandidateProfile parsed section profile: %s", final_profile.model_dump())
            logger.info(
    ...
    def _validate_candidate_profile_response(self, response_text: Optional[str]) -> Optional[CandidateProfile]:
        ...
        try:
            ...
            logger.warning("CandidateProfile JSON parse failed before repair: %s", exc)
            return None
...
"""
    ...
    def parse_vacancy(self, vacancy_text: str) -> VacancyProfile:
        ...
        try:
            prompt = PromptLoader.render("vacancy_analyzer", vacancy_text=vacancy_text)
            ...
            if not response.strip():
                logger.warning("VacancyAnalyzer fallback used: empty OpenAI section output")
                return normalize_vacancy_profile(self.fallback.parse_vacancy(vacancy_text), vacancy_text)

            parsed_profile = parse_vacancy_section_text(response)
            parsed_sections_count = len(re.findall(r"^\[[A-Z_]+\]\s*$", response, flags=re.MULTILINE))
            if parsed_sections_count == 0:
                logger.warning("VacancyAnalyzer fallback used: section output completely unparsable")
                return normalize_vacancy_profile(self.fallback.parse_vacancy(vacancy_text), vacancy_text)

            final_profile = normalize_vacancy_profile(parsed_profile, vacancy_text)
            logger.info(
            ...
            logger.error("VacancyAnalyzer fallback used after exception: %s", e)
            return normalize_vacancy_profile(self.fallback.parse_vacancy(vacancy_text), vacancy_text)
    ...
    def build_strategy(
        ...
        try:
            ...
            if not response.strip():
                logger.warning("CareerStrategy fallback used: empty model output")
                return build_fallback_strategy(
                    candidate_profile,
                    vacancy_profile,
                    candidate_preferences,
            ...
            if parsed_sections_count == 0:
                logger.warning("CareerStrategy fallback used: section output completely unparsable")
                return build_fallback_strategy(
                    candidate_profile,
                    vacancy_profile,
                    candidate_preferences,
    ...
    def generate_resume(
        ...
        if not self.enabled:
            logger.debug("Using deterministic resume fallback: OpenAI disabled")
        ...
        try:
            writer_context = build_resume_writer_context(candidate_profile, vacancy_profile, strategy_brief)
            ...
            if not writer_response.strip():
                logger.warning("ResumeWriter fallback used: empty writer output")
                return build_deterministic_resume(
                    candidate_profile,
                    vacancy_profile,
                    strategy_brief,
            ...
            generated_resume, writer_warnings, parsed_sections = parse_resume_writer_section_text(writer_response)
            if parsed_sections == 0 or len(generated_resume.resume_text.strip()) < 80:
                logger.warning("ResumeWriter fallback used: unparsable writer output")
                return build_deterministic_resume(
                    candidate_profile,
                    vacancy_profile,
                    strategy_brief,
            ...
            try:
                ...
                logger.debug("ResumeCritic raw section output:\n%s", critic_response)
                critic_report, critic_sections = parse_resume_critic_section_text(critic_response)
                if critic_sections == 0:
                    critic_fallback_used = True
                    critic_report = None
            except Exception as critic_error:
                logger.warning("ResumeCritic fallback used after exception: %s", critic_error)
                critic_fallback_used = True
